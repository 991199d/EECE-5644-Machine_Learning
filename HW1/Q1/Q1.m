...
figure(4);
plot(y(label==0),zeros(1,Nc(1)),'o','DisplayName','Class 0');
hold all;
plot(y(label==1),ones(1,Nc(2)),'o','DisplayName','Class 1');
ylim([-1 2]);
plot(repmat(tau(min_pFE_ind_LDA),1,2),ylim,'m--',...
    'DisplayName','Tau for Min. Error');
grid on;
xlabel('y');
title('Fisher LDA Projection of Data points and their true labels');
legend 'show';

% Plot ROC
figure(5);
plot(pFP_LDA,pTP_LDA,'DisplayName','ROC Curve');
hold all;
plot(min_FP_LDA,min_TP_LDA,'o','DisplayName',...
    'Estimated Min. Error');
xlabel('P(D=1|L=0) False Positive');
ylabel('P(D=1|L=0) True Positive');
...
% Plot tau
figure(6);
plot(tau,pFE_LDA,'DisplayName','Errors');
hold on;
